...
const Profile = () => {
  ...
  // Extract profile data with fallbacks for different API formats
  ...
  // Extract wallet data with fallbacks for different API formats
  ...
  // Debug logging
  console.log('Profile - Full Profile API Response:', profileData);
  console.log('Profile - Extracted Profile:', profile);
  console.log('Profile - Full Wallet API Response:', walletData);
  console.log('Profile - Extracted Wallet:', wallet);

  // Extract name fields with fallbacks
  const firstName = profile.firstName || profile.first_name || '';
  const lastName = profile.lastName || profile.last_name || '';
  const fullName = profile.name || `${firstName} ${lastName}`.trim() || 'N/A';
  
  // Extract email
  const email = profile.email || 'N/A';
  
  // Extract phone
  const phone = profile.phone || profile.phoneNumber || profile.phone_number || null;
  
  // Extract member since date with fallbacks
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      ...
                      {kycStatus ? kycStatus.charAt(0).toUpperCase() + kycStatus.slice(1) : 'Not submitted'}
                    </span>
                  </div>
